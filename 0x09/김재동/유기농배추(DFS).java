...
public class Ox10_Q2_1 {
            ...
            for(int sero = 0; sero <n; sero++) {
                for(int garo = 0; garo<m; garo++) {
                    if(field[sero][garo]==1 && !visited[sero][garo]) {
                        int size = dfs(sero,garo,field,visited,n,m);
                        fieldArea.add(size);
                    }
                }
            }// 배추밭 테스트 끝
    ...
    // 깊이 우선 탐색 (DFS)
    public static int dfs(int x, int y, int[][] field, boolean[][]visited, int n, int m ) {
        // 새로운 배추흰지렁이가 필요한 위치 stack 생성
        Stack<LocationQ2> fieldStk = new Stack<LocationQ2>();
        fieldStk.push(new LocationQ2(x, y));
        // 방문 여부 체크, 지렁이 한마리로 관리 가능한 넓이 체크
        visited[x][y] = true;
        int size = 0;
        //  deq가 비었다 -> 더이상 연결 되지 않는 다는 의미.
        while(!fieldStk.isEmpty()) {
            LocationQ2 nowLoc = fieldStk.pop();
            size++;
            for(int i = 0; i<4; i++) {
                ...
                if(tempX >=0 && tempY >=0 && tempX <n && tempY<m) {
                    ...
                    if(field[tempX][tempY]==1&&!visited[tempX][tempY]) {
                        fieldStk.add(new LocationQ2(tempX, tempY));
