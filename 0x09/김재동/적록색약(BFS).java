...
public class OxO9_Q3_1 {
    ...
    public static void main(String[] args)  throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (!visited[i][j]) {
                    int size = bfs(i, j, grid, visited, n);
                    gridArea.add(size);
                }
            }
        }
    ...
    //너비 우선 탐색 (BFS)
    public static int bfs(int x, int y, String [][] grid, boolean[][]visited, int n) {
        Deque<LocationQ3> gridDeq = new ArrayDeque<LocationQ3>();
        gridDeq.add(new LocationQ3(x, y));
        visited[x][y] = true;
        int size = 0;
        String temp = grid[x][y];
        //  deq가 비었다 -> 더이상 연결 되지 않는 다는 의미.
        while(!gridDeq.isEmpty()) {
            LocationQ3 nowLoc = gridDeq.pollFirst();
            size++;
            for(int i = 0; i<4; i++) {
                ...
                if(tempX >=0 && tempY >=0 && tempX <n && tempY<n) {
                    ...
                    if(grid[tempX][tempY].equals(temp)&&!visited[tempX][tempY]) {
                        gridDeq.add(new LocationQ3(tempX, tempY));
